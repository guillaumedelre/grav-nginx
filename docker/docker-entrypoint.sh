...
_grav_conf () {
  local OUTFILE=/etc/php/7.3/fpm/pool.d/grav.conf
  ...
  echo "" >> $OUTFILE
  echo "listen = /run/php/php7.3-fpm.sock" >> $OUTFILE
  echo "" >> $OUTFILE
...
_http_grav_site () {
  ...
  echo "      fastcgi_pass unix:/run/php/php7.3-fpm.sock;" >> $OUTFILE
...
if [[ "$1" = 'grav' ]]; then
  ...
  # update mime.types
  _mime_types
  # start Nginx and php7.3-fpm
  /etc/init.d/nginx start
  /etc/init.d/php7.3-fpm start
  # set: cgi.fix_pathinfo=0 in /etc/php/7.3/fpm/php.ini
  ...
  _nginx_conf
  # set /etc/php/7.3/fpm/pool.d/grav.conf
  _grav_conf
  # install grav
